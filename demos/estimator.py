...
# Just to check that problem in Eq. (12)-(13) gives same solution as problem in Eq. (7)-(8) when there is no horizon
class EstimatorEquivalent():
    def __init__(self, pin_robot, nc, nc_delta_f, frameId, baumgarte_gains, pinRefRame='LOCAL'):
        self.pin_robot = pin_robot
        self.nc = nc
        self.nc_delta_f = nc_delta_f
    ...
    def estimate(self, q, v, a, tau, df_prior, F_mes):
        ...
        self.b = np.concatenate([h - tau, -alpha0], axis=0)
        self.A[:self.nv, :self.nv] = -M.copy()
        self.A[:self.nv, self.nv:self.nv+self.nc] = J1.T.copy()

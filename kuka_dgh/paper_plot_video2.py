...
print("Load data 1...")
r1 = DataReader(data_path+'config_REAL_2023-09-20T12:07:40.864401_FL_pertubation.mds') 
print("Load data 2...")
# r2 = r1 
r2 = DataReader(data_path+'config_REAL_2023-09-20T12:01:58.881430_FL_DF_PM_pertubation.mds') 
...
delta_F =  analysis_utils.moving_average_filter(r2.data['delta_f'][N_START:N].copy(), 100) 
...
ax2.set_ylabel(r'$\Delta F$ Estimate (N)', fontsize=30)
...
if PART == 1:
    line_f_r1, = ax1.plot(time_lin[0:1], force_3d_1[0:1, 2], animated=True, color=color_list[1], linewidth=4, label=label1, alpha=0.8)
    line_df_r1, = ax2.plot(time_lin[0:1], np.zeros(time_lin[0:1].shape), animated=True, color=color_list[1], linewidth=4, label=label1, alpha=0.8)
    line = [line_f_r1, line_df_r1]
    
if PART == 2:
    line_f_r1, = ax1.plot(time_lin[:], force_3d_1[:, 2], animated=True, color=color_list[1], linewidth=4, label=label1, alpha=0.8)
    line_f_r2, = ax1.plot(time_lin[0:1], force_3d_2[0:1, 2], animated=True, color=color_list[2], linewidth=4, label=label2, alpha=0.8)


    line_df_r1, = ax2.plot(time_lin[:], np.zeros(time_lin.shape), animated=True, color=color_list[1], linewidth=4, label=label1, alpha=0.8)
    line_df_r2, = ax2.plot(time_lin[0:1], delta_F[0:1], animated=True, color=color_list[2], linewidth=4, label=label2, alpha=0.8)
...
if PART == 1:
    fig.text(0.45, 0.3, 'No estimation',transform=fig.transFigure, fontdict={'size':50})
...
def animate(t):
    ...
    if PART == 1:
        ...
        line[1].set_data(time_lin[0:t:SKIP], np.zeros(time_lin[0:t:SKIP].shape))
    if PART == 2:
        line[2].set_data(time_lin[0:t:SKIP], force_3d_2[0:t:SKIP, 2])
        line[3].set_data(time_lin[0:t:SKIP], delta_F[0:t:SKIP])
...
ani.save('animation_FL_vs_DF_perturbation_' + str(PART) + '.mp4')

...
def compute_pos_error_traj(r):
    ...
    return (np.sqrt((p_mea[:, 0] - r.data['target_position_x'][N_START:N,0])**2 + (p_mea[:, 1] - r.data['target_position_y'][N_START:N,1])**2)).reshape((-1, 1))



pos_error_1 = analysis_utils.moving_average_filter(compute_pos_error_traj(r1).copy(), 100)
pos_error_2 = analysis_utils.moving_average_filter(compute_pos_error_traj(r2).copy(), 100)
pos_error_3 = analysis_utils.moving_average_filter(compute_pos_error_traj(r3).copy(), 100)

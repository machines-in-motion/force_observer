#ifndef __force_observer_estimator_tau__
#define __force_observer_estimator_tau__
...
struct TorqueEstimatorData {
  ...
  explicit TorqueEstimatorData(Model* const model)
      : pinocchio(pinocchio::Data(model->get_pinocchio())),
        F(model->get_nc()),
        delta_tau(model->get_pinocchio().nv),
        ...
        A(model->get_pinocchio().nv + model->get_nc(), 
          model->get_pinocchio().nv + model->get_nc() + model->get_pinocchio().nv),
        g(model->get_n_tot()),
  ...
  virtual ~TorqueEstimatorData() {}
  
  pinocchio::Data pinocchio; //!< Pinocchio data

  VectorXd F;       //!< Force
  VectorXd delta_tau; //!< Force offset estimate
...
class TorqueEstimator{
 ...
 public:
  ...
  typedef typename Eigen::VectorXd VectorXd;
  typedef typename Eigen::MatrixXd MatrixXd;
  typedef TorqueEstimatorData Data;
  ...
  /**
   * @brief Computes the torques estimate from measurements and prior
   *
   * @param[in] data       Torque estimator data
  ...
  void estimate(const boost::shared_ptr<TorqueEstimatorData>& data, 
                ...
                const Eigen::Ref<const VectorXd>& dtau_prior,
...
#endif  // __force_observer_estimator_tau__
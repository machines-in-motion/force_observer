#ifndef __force_observer_mh_varying_stimator__
#define __force_observer_mh_varying_stimator__
...
struct MHVaryingForceEstimatorData {
  ...
  explicit MHVaryingForceEstimatorData(Model* const model)
      : pinocchio(pinocchio::Data(model->get_pinocchio())),
        ...
        A(model->get_pinocchio().nv + model->get_nc(), 
          model->get_pinocchio().nv + model->get_nc() + model->get_nc()),
        g(model->get_n_tot()),
  ...
  virtual ~MHVaryingForceEstimatorData() {}
...
class MHVaryingForceEstimator{
 ...
 public:
  ...
  typedef typename Eigen::VectorXd VectorXd;
  typedef typename Eigen::MatrixXd MatrixXd;
  typedef MHVaryingForceEstimatorData Data;
...
#endif  // __force_observer_mh_varying_estimator__